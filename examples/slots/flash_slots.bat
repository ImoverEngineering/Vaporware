@echo off
:: flash_slots.bat - flash slot machine firmware to a vape via ST-Link
:: Run build_slots.bat first, then python gen_direct_flash.py, then this.

cd /d "%~dp0"

echo.
echo  ========================================
echo   Vape Flasher - Slot Machine
echo  ========================================
echo.

if not exist "build\slots.bin" (
    echo  ERROR: build\slots.bin not found. Run build_slots.bat first.
    pause
    exit /b 1
)

for %%A in ("build\slots.bin") do echo  Firmware: %%~zA bytes  [build\slots.bin]
echo.
